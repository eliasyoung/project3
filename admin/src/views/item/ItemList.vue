...
import { getItem, itemDeleteById } from "@/http/item";
export default {
  setup() {
    ...
    //跳转编辑页面
    const onEdit = (data) => {
      router.push(`/items/edit/${data._id}`);
      // router.push("items/edit/" + data._id);
      // router.push({ path: "/items/edit", query: { id: data._id } });
    };
    // 删除数据
    const onDelete = async (data) => {
      console.log("beforeDelete", data);
      const res = await itemDeleteById(data._id);
      console.log(res);
      ElMessage.success({
